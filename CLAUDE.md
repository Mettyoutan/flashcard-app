...
**Learning notes (keep updated):** `../learns/react-frontend-notes.md` (workspace root, not project-scoped — shared across whatever else lands in `workspace/learns/`) is the user's own React/frontend reference doc, written for himself to re-read later, not project documentation. When a genuinely new, reusable React/Next.js/frontend concept comes up while working in `web/` (a pattern, a gotcha, a debugging habit worth keeping) — the kind of thing this file's own inline commentary already tends to capture in prose — add a short entry there too, in the same style as its existing sections. Don't duplicate this CLAUDE.md's own status/architecture notes into it; it's concepts-to-remember, not a changelog.
